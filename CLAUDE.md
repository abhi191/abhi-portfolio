...
- `InteractiveGrid.tsx` — canvas dot-grid background; reads `--accent` live
  ...
  updates when the switcher changes without a re-render. The grid geometry is
  rebuilt by a `ResizeObserver` on the canvas (not a window `resize` listener)
  — see gotcha #6.
- `ProjectPageRedesign.tsx` — the editorial case-study layout used by **all**
  project detail pages. Text-only hero (back link → title → overview → mono
  metadata → accent metrics band; no header image), sticky numbered TOC
  sidebar, `space-y-32` section rhythm with an accent mono section index, and
  an "All projects" end-of-page nav button (`font-black`, hairline top border)
  that mirrors the top back link. `RenderBlock` renders every content-block
  type and keeps all block-container palettes on the single accent + ink
  tokens (no per-block rainbow hues).
...
6. **Canvas grids must rebuild on font reflow, not just window resize.** The
   `InteractiveGrid` dot-grid intermittently rendered sparse/mis-centered on a
   cold homepage load: it measured the canvas once at mount and only rebuilt on
   a window `resize` event. But the `min-h-[82dvh]` hero grows when the Satoshi
   web font swaps in *after* mount — a reflow that fires **no** `resize` event —
   so the grid kept a layout sized for the pre-font (shorter) canvas. Fix: drive
   `initGrid()` from a `ResizeObserver` on the canvas (catches any box-size
   change, font swap included), guard `initGrid` against a 0×0 measurement, and
   use `ctx.setTransform(dpr,0,0,dpr,0,0)` instead of `ctx.scale(dpr,dpr)` so
   the now-more-frequent re-inits don't compound the DPR scale each time.

7. **The `.accent-word` ink stroke is display-scale only — don't apply it to
   body/UI text.** The "View case study" card link used `.accent-hover-text`,
   which on hover set both the accent color *and* a `-webkit-text-stroke:
   0.6px` ink outline. At body size that hairline outline reads as an odd
   double-outlined button, not an accent link. The stroke treatment belongs to
   the giant hero word (`.accent-word`) where it aids legibility of light
   accents; for links/labels use accent color + weight only. The hover-stroke
   rule was removed from `index.html`; `.accent-hover-text` now only changes
   color.

8. **Reuse the back-link idiom for the end-of-page "All projects" nav.** The
   project detail page has two returns-to-home affordances on purpose: a small
   muted breadcrumb at the top (`text-sm text-brand-dark/40`) and a large
   `font-black text-2xl/3xl` "All projects" button after the last section
   (hairline top border, `LeftArrowIcon` sliding left on hover). Both just set
   `window.location.hash = '#/'`. Keep the same `LeftArrowIcon` + accent-hover
   pattern if you add more nav — don't introduce a different arrow/hover style.
