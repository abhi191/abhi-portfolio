    ...
    <nav className="space-y-1">
      ...
      {sections.map((section) => (
        <button
          ...
          className={`text-left w-full block py-2 px-4 text-sm font-medium transition-all duration-300 border-l-2 ${activeSection === section.id
            ? 'border-brand-accent text-brand-accent translate-x-1'
...
const ProjectPageRedesign: React.FC<{ project: Project }> = ({ project }) => {
  ...
  return (
    <article className="min-h-screen bg-brand-background">
      ...
      <div className="container mx-auto px-6 sm:px-8 md:px-12 lg:px-20 xl:px-24 pt-32 pb-24">
        ...
        <header className="mb-24 max-w-5xl">
          <AnimateOnScroll>
            ...
            <button
              ...
              className="group inline-flex items-center gap-2 text-sm font-medium text-brand-dark/40 hover:text-brand-accent mb-8 transition-colors"
            >
            ...
            {/* Massive Title */}
            <h1 className={`text-5xl md:text-7xl lg:text-8xl font-black tracking-tighter leading-[0.9] text-brand-dark mb-8 ${project.titleColor ? `text-highlight-${project.titleColor}-text` : ''}`}>
              {project.title}
            </h1>

            {/* Impact Abstract (Metrics Row) */}
            {project.metrics && project.metrics.length > 0 && (
              <div className="flex flex-wrap gap-8 md:gap-16 border-y border-brand-dark/10 py-8 mb-10">
                {project.metrics.map((metric, idx) => (
                  <div key={idx} className="flex flex-col">
                    <span className="text-3xl md:text-4xl font-bold text-brand-dark tracking-tight">{metric.value}</span>
                    <span className="text-sm font-medium text-brand-dark/50 uppercase tracking-wider mt-1">{metric.label}</span>
                  </div>
                ))}
              </div>
            )}

            <p className="text-xl md:text-2xl text-brand-dark/70 leading-relaxed max-w-3xl">
              {project.overview}
            </p>

            {/* Metadata Tags */}
            <div className="mt-8 flex flex-wrap gap-4 text-sm font-medium text-brand-dark/60">
              {project.details.map((detail, index) => (
                <span key={index} className="bg-brand-card px-3 py-1.5 rounded-full border border-brand-dark/5">
                  {detail.value}
                </span>
              ))}
            </div>
          ...
          {!project.hideHero && project.heroImage && (
            <AnimateOnScroll delay={200}>
              <div className="mt-16 rounded-3xl overflow-hidden aspect-w-16 aspect-h-9 bg-brand-card shadow-2xl shadow-brand-dark/5">
            ...
            <div className="space-y-32">
              {sectionsWithIds.map((section, sectionIndex) => (
                <section key={section.id} id={section.id} className="scroll-mt-32">
                  <AnimateOnScroll>
                    <h2 className="text-3xl md:text-4xl font-bold text-brand-dark mb-12 flex items-center gap-4">
                      {/* Optional: Add a subtle line or accent */}
                      <span className="w-12 h-1 bg-brand-dark/10 rounded-full"></span>
                      {section.title}
                    </h2>
