...
const InteractiveGrid: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const initGrid = () => {
            ...
            // Guard against measuring before layout settles (e.g. a font
            // reflow mid-mount) — a zero dimension would build a degenerate grid.
            ...
            // setTransform (not scale) so repeated re-inits don't compound.
        ...
        // A ResizeObserver on the canvas catches every box-size change —
        // window resizes AND late layout shifts like the web-font swap that a
        // window 'resize' event would miss (the cause of the sparse/misaligned
        // grid seen on a cold homepage load).
        ...
        // Reduced motion: draw the grid once (redrawn on resize), no physics loop
        if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
            ...
            drawStatic();
            const staticObserver = new ResizeObserver(drawStatic);
